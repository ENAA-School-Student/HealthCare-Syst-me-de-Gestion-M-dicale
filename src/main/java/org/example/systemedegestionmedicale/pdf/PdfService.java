package org.example.systemedegestionmedicale.pdf;
...
public class PdfService {
    ...
    public byte[] generateRendezVousPdf(List<RendezVouResponseDto> rdvs) {
        ...
        try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            for (RendezVouResponseDto r : rdvs) {
                table.addCell(String.valueOf(r.getDateRendezVous()));
                table.addCell(String.valueOf(r.getPatientId()));
                table.addCell(String.valueOf(r.getMedecinId()));
                table.addCell(String.valueOf(r.getStatusRendezVou()));
            }
    ...
    public byte[] generateDossierMedicalPdf(DossierMedicalResponseDto dm) {
        try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            Document document = new Document();
            PdfWriter.getInstance(document, out);
            document.open();
            ...
            document.add(new Paragraph("Patient ID: " + dm.getPatientId()));
            document.add(new Paragraph(" "));
